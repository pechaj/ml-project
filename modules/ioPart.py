...
def loadData(filename, block_num):
    """
    ...
    ecg_signal_file = ecg_files[0] if ecg_files else None  # Select the first match if available

    # Find the EDA file dynamically
    eda_files = glob.glob(os.path.join(base_path, f"{block_num}_gsr_ppg_*.csv"))
    eda_signal_file = eda_files[0] if eda_files else None  # Select the first match if available
...
def saveData(data, signal_type, dest_dir1, dest_dir2, part_num, block_num):
    ...
    if part_num <= 11:
        if block_num in setup1LowLoad:
            dest_dir = dest_dir1
        elif block_num in setup1HighLoad:
            dest_dir = dest_dir2
        else:
