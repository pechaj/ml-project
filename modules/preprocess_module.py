...
def preprocessDataset(ecg_signal_full: pd.DataFrame, eda_signal_full: pd.DataFrame, fs: int) -> tuple[np.ndarray | None, np.ndarray | None]:
...
def preprocessSignalECG(signal: pd.DataFrame, fs: int) -> np.ndarray | None:
    ...
    try:
        ...
        signal, rpeaks = ecg_peaks(ecg_signal_processed, sampling_rate=fs)
...
def preprocessSignalEDA(signal: pd.DataFrame, fs: int) -> np.ndarray | None:
    """
    ...
    Returns:
        np.ndarray: The processed and normalized EDA signal.
    """
